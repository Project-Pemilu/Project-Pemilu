...
export default function HomePage() {
  ...
  return (
    ...
    >
      <h2
        className="text-4xl tracking-widest text-white text-center uppercase font-bold py-5"
        style={{ color: theme.data[theme.currentTheme].fontColor }}
      >
        ...
        {candidates.map((candidate, index) => (
          <div key={candidate.id} className="group flex flex-col items-center">
            ...
            >
              ...
              <p
                className="text-sm text-gray-300 text-center my-2 px-5 h-24"
                style={{ color: theme.data[theme.currentTheme].cardText }}
              >
              ...
              <p
                ...
                style={{
                  backgroundColor:
                    theme.data[theme.currentTheme].buttonContainer,
                  color: theme.data[theme.currentTheme].fontColor,
                }}
              >
                {translations[currentLanguage].candidates[index]?.votes}:{" "}
                {candidate.totalVote}
              </p>
            </div>
          </div>
